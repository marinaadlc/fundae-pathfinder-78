...
const Students = () => {
  ...
  const getSortIcon = (field: keyof Student) => {
    ...
    return sortDirection === "asc" ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />;
  };
  const filteredAndSortedStudents = mockStudents.filter(student => {
  ...
  return <div className="space-y-6">
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input id="name" placeholder="Introduce el nombre del alumno" />
              </div>
              <div className="grid gap-2">
                <label htmlFor="email" className="text-sm font-medium">
                  Email
                </label>
                <Input id="email" type="email" placeholder="alumno@empresa.com" />
              </div>
            </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
              ...
              <Input placeholder="Buscar alumnos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
      ...
      <Card>
        <CardContent className="p-0">
            ...
            <table className="w-full">
              <thead className="border-b bg-muted/50">
                <tr>
                  ...
                  <th className="text-left p-4 font-medium text-muted-foreground">
                    <button className="flex items-center gap-2 hover:text-foreground transition-colors" onClick={() => handleSort("completedFormations")}>
                      Formaciones realizadas
                      {getSortIcon("completedFormations")}
                    </button>
                  </th>
                  <th className="text-left p-4 font-medium text-muted-foreground">
                    <button className="flex items-center gap-2 hover:text-foreground transition-colors" onClick={() => handleSort("currentCourse")}>
                      Formación en curso
                      {getSortIcon("currentCourse")}
                    </button>
                  </th>
                  <th className="text-left p-4 font-medium text-muted-foreground">
                    <button className="flex items-center gap-2 hover:text-foreground transition-colors" onClick={() => handleSort("lastActivity")}>
                      Última actividad
              ...
              <tbody>
                {filteredAndSortedStudents.map(student => <tr key={student.id} className="border-b hover:bg-muted/30 transition-colors">
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-foreground">
                      {student.status === "inactive" ? "-" : student.currentCourse ? <button onClick={() => navigate(`/action/${student.id}`)} className="text-primary hover:text-primary/80 underline underline-offset-2 transition-colors">
          ...
          {filteredAndSortedStudents.length === 0 && <div className="text-center py-12">
      ...
      {/* Pagination */}
      {filteredAndSortedStudents.length > 0 && <div className="flex items-center justify-between text-sm text-muted-foreground">
